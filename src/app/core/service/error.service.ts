...
export class ErrorService {
  ...
  handleError(error: any): void {
    ...
    if (error instanceof HttpErrorResponse) {
      ...
      console.log(this.getModalType(error.status));
  ...
  showModalByStatusCode(statusCode: number, customMessage?: string): void {
    ...
    this.modalService.showModalByStatusCode(statusCode.toString(), customMessage || message);
  }
  ...
  handleNetworkError(error: any): void {
    if (!navigator.onLine) {
      this.showModal(this.errorMessages.internetNotAvailable, 'error', 'NETWORK_ERROR', 'Network Error');
    } else if (error.name === 'TimeoutError') {
      this.showModal('Request timeout. Please try again.', 'warning', 'TIMEOUT', 'Request Timeout', 4000);
    } else {
  ...
  handleLoginError(error: any): void {
    if (error.status === 401) {
      this.showModal('Invalid email or password. Please try again.', 'error', '401', 'Login Failed', 5000);
    } else {
      this.handleError(error);
    }
  }
  ...
  handleValidationError(errors: string[]): void {
    if (errors && errors.length > 0) {
      ...
      this.showModal(errorMessage, 'error', '422', 'Validation Error', 5000);
    }
  }
