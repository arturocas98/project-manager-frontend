...
} from '../../shared/models/projects-models/message-models';
...
export class ProjectService {
  ...
  getProjectSummary(projectId: number): Observable<ProjectSummaryData | null> {
    if (!this.projectSummaryCache.has(projectId)) {
      ...
      this.apiService
        ...
        .pipe(tap(summary => subject.next(summary)))
        .subscribe();
    } else {
  ...
  updateMember(
    projectId: number,
    memberId: number,
    projectMemberUpdateData: ProjectMemberUpdateRequest
  ): Observable<any> {
  ...
  getUnassignedUsers(projectId: number): Observable<unassignedUsersData[]> {
    return this.apiService.get<unassignedUsersData[]>(`projects/${projectId}/unassigned-users?page=1&per_page=1000`);
  }
  ...
  createComment(projectId: number, taskId: number, description: string, file?: File): Observable<CommentResponse> {
    ...
    return this.apiService
      .postFull<CommentResponse>(`projects/${projectId}/incidences/${taskId}/comments`, formData, {
  ...
  updateComment(
    ...
    return this.apiService
      ...
      }>(`projects/${projectId}/incidences/${taskId}/comments/${commentId}`, { description })
      .pipe(
  ...
  deleteComment(projectId: number, taskId: number, commentId: number): Observable<void> {
    ...
    return this.apiService.delete<void>(`projects/${projectId}/incidences/${taskId}/comments/${commentId}`);
  }

  // --- PROJECT MESSAGES (CHAT) ---

  getProjectMessages(projectId: number): Observable<MessageResource[]> {
    return this.apiService.get<any>(`projects/${projectId}/messages`).pipe(map(res => res.data || res));
  }

  getProjectFiles(projectId: number): Observable<any[]> {
    return this.apiService.get<any>(`projects/${projectId}/files`).pipe(map(res => res.data || res));
  }
  ...
  createProjectMessage(projectId: number, payload: MessageRequest, file?: File): Observable<MessageResource> {
    if (file) {
      ...
      return this.apiService
        .postFull<any>(`projects/${projectId}/messages`, formData, { omitContentType: true })
        .pipe(map(res => res.data || res));
    }

    return this.apiService.post<any>(`projects/${projectId}/messages`, payload).pipe(map(res => res.data || res));
  }
  ...
  updateProjectMessage(
    ...
    return this.apiService
      .put<any>(`projects/${projectId}/messages/${messageId}`, payload)
      .pipe(map(res => res.data || res));
  }
