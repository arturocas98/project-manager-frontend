...
export class LocalStorageService {
  ...
  setValueToLocal(key: string, value: any): void {
    ...
    if (this.isLoggingEnabled) {
      console.log(
        `setValueToLocal: ${typeof value} - ${fullKey} - ${stringValue}`,
      );
    }
    ...
    try {
      ...
      console.error(
        `Error al guardar en localStorage (key: ${fullKey}):`,
        error,
      );
  ...
  getValueFromLocal<T>(key: string): T | null {
    ...
    if (this.isLoggingEnabled) {
      console.log(
        `getValueFromLocal: ${typeof rawValue} - ${fullKey} - ${rawValue}`,
      );
    }
