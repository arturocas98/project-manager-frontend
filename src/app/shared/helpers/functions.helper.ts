import { Constants, DATE_FORMATS, ERROR_FORM_VALIDATION, NUMBERS, REGEX } from '../constants/constants';
import { AbstractControl, FormGroup, ValidationErrors, ValidatorFn } from '@angular/forms';
import { format, parseISO, isDate, isAfter, isBefore, isValid, subMonths } from 'date-fns';
...
export function convertDateStringsToDatesImmutable<T>(input: T): T {
  if (Array.isArray(input)) {
    return input.map(item => convertDateStringsToDatesImmutable(item)) as unknown as T;
  }
...
export function convertDatesToStringsImmutable<T>(input: T): T {
  if (Array.isArray(input)) {
    return input.map(item => convertDatesToStringsImmutable(item)) as unknown as T;
  }
...
export function replaceNestedObjectsWithId(obj: any, excludeKeys: string[] = []): any {
  ...
  if (obj !== null && typeof obj === 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (excludeKeys.includes(key)) {
        newObj[key] = value;
      } else if (value && typeof value === 'object' && 'id' in value && Object.keys(value).length > 0) {
        newObj[key] = value.id;
      } else {
...
export function extractFilesToFormData(obj: any, formData: FormData = new FormData(), path: string[] = []): FormData {
    ...
    for (const key of Object.keys(obj)) {
      ...
      if (value instanceof File) {
        ...
        value.forEach((item, index) => {
          extractFilesToFormData(item, formData, [...currentPath, index.toString()]);
        });
...
function compareDates(dateA: string, dateB: string, operation: ComparisonOperator): boolean {
...
export function flattenErrors(errors: Record<string, string | string[]>): string[] | null {
  const result: string[] = [];
...
export function isFlatObject(obj: Record<string, any>): boolean {
  return Object.values(obj).every(value => typeof value !== 'object' || value === null || Array.isArray(value));
}
...
export function convertString(
  ...
  if (properties.camel) {
    str = str.toLowerCase().replace(/[-_\s]+(.)?/g, (_, group1) => (group1 ? group1.toUpperCase() : ''));
  }
  return str;
}

export function fixNumber(value: number | string, decimals: number = NUMBERS.TWO): string {
  return Number(value).toFixed(decimals);
}

export function getLastMonthsNames(defaultLength: number = NUMBERS.SIX): string[] {
  const today = new Date();
  return Array.from({ length: defaultLength }, (_, i) => {
    const month = format(subMonths(today, defaultLength - NUMBERS.ONE - i), 'MMMM', { locale: es });
